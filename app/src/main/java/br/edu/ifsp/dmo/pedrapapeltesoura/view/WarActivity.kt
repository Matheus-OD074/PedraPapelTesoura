...
class WarActivity : AppCompatActivity() {
    ...
    private fun battle(){
        ...
        if(weaponPlayer1 != null && weaponPlayer2 != null){
            ...
            Toast.makeText(this,"$name${getString(R.string.choose_gum_player)}", Toast.LENGTH_LONG).show()
        }
    }
    ...
    private fun configResultLauncher(){
        resultLaucher = registerForActivityResult(ActivityResultContracts.StartActivityForResult())
        {
            result ->
    ...
    private fun openBundle(){
        ...
        if (extras != null){
            val p1 = extras.getString(Constants.KEY_PLAYER1)
            val p2 = extras.getString(Constants.KEY_PLAYER2)
